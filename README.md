...
- `decord` does not currently publish macOS (Apple Silicon) wheels. Video utilities that load frames from `.mp4` using `decord` will be unavailable on macOS; use a JPEG frame folder input instead.
- `triton` does not currently publish macOS wheels. Where SAM3 uses Triton kernels (e.g., for EDT used in some interactive sampling utilities), it will fall back to a slower CPU implementation on macOS.
- If you use `uv` on macOS, PyTorch will be resolved from PyPI (CPU/Metal build). CUDA wheels are Linux/Windows-only; for CUDA installs, follow the PyTorch install command for your platform.
