...
class Sam3Processor:
    ...
    def __init__(
        ...
        # Normalize device string for environments without CUDA (e.g., macOS).
        if isinstance(device, torch.device):
            device = str(device)
        if device.startswith("cuda") and not torch.cuda.is_available():
            device = "mps" if getattr(torch.backends, "mps", None) and torch.backends.mps.is_available() else "cpu"
        if device.startswith("mps") and not (getattr(torch.backends, "mps", None) and torch.backends.mps.is_available()):
            device = "cpu"
