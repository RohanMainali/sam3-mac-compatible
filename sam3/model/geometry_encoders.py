...
def concat_padded_sequences(seq1, mask1, seq2, mask2, return_index: bool = False):
    ...
    # `torch._assert_async` is not implemented on MPS. These checks are for
    # correctness/debugging only, so we fall back to a CPU-side assert when needed.
    if mask1.device.type == "mps":
        assert is_right_padded(mask1.detach().cpu()).item()
    else:
        torch._assert_async(is_right_padded(mask1))

    if mask2.device.type == "mps":
        assert is_right_padded(mask2.detach().cpu()).item()
    else:
...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _encode_points(self, points, points_mask, points_labels, img_feats):
        ...
        # Fast-path for empty prompts (common in text-only grounding).
        # Some backends (notably MPS) can error on ops like grid_sample with empty tensors.
        if n_points == 0:
            empty = torch.zeros(
                (0, bs, self.d_model),
