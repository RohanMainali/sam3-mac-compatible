...
def edt_triton(data: torch.Tensor):
    ...
    # Fallback path: use OpenCV on CPU (works on macOS / CPU / MPS).
    # This is slower than the Triton kernel but keeps the model usable.
    import cv2
    import numpy as np

    device = data.device
    data_u8 = (data != 0).detach().to("cpu").numpy().astype(np.uint8)

    B, _, _ = data_u8.shape
    out = np.empty_like(data_u8, dtype=np.float32)
    for b in range(B):
        out[b] = cv2.distanceTransform(data_u8[b], cv2.DIST_L2, 0)
