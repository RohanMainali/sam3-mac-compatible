...
class TransformerDecoder(nn.Module):
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        H, W = feat_size
        ref_device = reference_boxes.device
        boxes_xyxy = box_cxcywh_to_xyxy(reference_boxes).transpose(0, 1)
        bs, num_queries, _ = boxes_xyxy.shape
        # Cache needs to be device-aware: the model can be moved CPU<->MPS/CUDA.
        stored_device = getattr(self, "compilable_stored_device", None)
        if (
            self.compilable_cord_cache is None
            or stored_device is None
            or stored_device != ref_device
            or getattr(self, "compilable_stored_size", None) != (H, W)
        ):
            self.compilable_cord_cache = self._get_coords(H, W, ref_device)
            self.compilable_stored_size = (H, W)
            self.compilable_stored_device = ref_device

        if torch.compiler.is_dynamo_compiling() or (
            self.compilable_stored_size == (H, W)
            and getattr(self, "compilable_stored_device", None) == ref_device
        ):
            ...
            key = (feat_size, str(ref_device))
            if key not in self.coord_cache:
                self.coord_cache[key] = self._get_coords(H, W, ref_device)
            coords_h, coords_w = self.coord_cache[key]
