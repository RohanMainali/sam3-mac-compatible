...
from sam3.env_utils import get_hf_token, load_env_file
...
def _setup_device_and_mode(model, device, eval_mode):
    ...
    # Normalize requested device and fall back safely on platforms without CUDA.
    ...
    if device.startswith("mps") and not mps_available:
        device = "cpu"
...
def download_ckpt_from_hf():
    ...
    if not token:
        raise RuntimeError(
            "Missing Hugging Face auth token for gated repo 'facebook/sam3'. "
            "Create a `.env` file (see `.env.example`) with `HF_TOKEN=...`, "
            "or run `hf auth login`, and ensure your account has been granted access."
        )
...
def build_sam3_video_model(
    ...
    # Import lazily to avoid pulling in CUDA-only autocast decorators on non-CUDA platforms.
    from sam3.model.sam3_video_inference import Sam3VideoInferenceWithInstanceInteractivity
